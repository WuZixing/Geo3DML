...
|27.2||ToolName|创建该地质模型所使用的软件工具的名称。|1|字符串|自由文本。|
|27.3||ToolVersion|创建该地质模型所使用的软件工具的版本号。|1|字符串|自由文本。|
|27.4||SpatialReferenceSystem|空间数据参照系。|0..1|类|geo3dml:AbstractSpatialReferenceSystem具体类型的实例。|
